...
double GlobalTimeStep (double *Q, double *H, double *dx);
...
#elif INITIAL_CONDITION == 100
    for (i=0; i<nCells; i++){
        Q[i] = (x[i] < L/2. ? 0. : 0.);
        H[i] = (x[i] < L/2. ? 1.0 : 0.5); //1 : 0.30179953
        z[i] = (x[i] < L/2. ? 0. : 0.5); // 0. : 0.05
    }
#elif INITIAL_CONDITION == 101
    for (i=0; i<nCells; i++){
        Q[i] = (x[i] < L/2. ? 0. : 0.);
        H[i] = (x[i] < L/2. ? 1.0 : 0.); //1 : 0.30179953
        z[i] = (x[i] < L/2. ? 0. : 1.5); // 0. : 0.05
    }
#else
    for (i=0; i<nCells; i++){
...
//        }
        ...
        dt = GlobalTimeStep(Q,H,dx);

        if (t+dt>simTime) dt = simTime-t;

//        if (IterCounter % 1000 ==0) printf("t=%.15e\tdt=%.15e\n",t,dt);
//        getchar();
        ...
        for (i=1; i<nCells-1; i++)
        {
            ...
            alpha1 = (cAux-uAux)*(H[i]-H[i-1])/2./cAux + (Q[i]-Q[i-1])/2./cAux;
            alpha2 = (cAux+uAux)*(H[i]-H[i-1])/2./cAux - (Q[i]-Q[i-1])/2./cAux;
            ...
            if (lambda1>0.){
                lambda = fabs(Entropy_fix(lambda1,lambdaL1,lambdaR1,1));
                dH[i] += dt/dx[i] * (lambda*alpha1-source);
                dQ[i] += dt/dx[i] * (lambda*alpha1-source)*lambda1;
            }
            if (lambda2>0.){
                lambda = fabs(Entropy_fix(lambda2,lambdaL2,lambdaR2,1));
                dH[i] += dt/dx[i] * (lambda*alpha2+source);
                dQ[i] += dt/dx[i] * (lambda*alpha2+source)*lambda2;
            }
...
///////////////////////////////////////////////////////////////////////
            ...
            if (lambda1<0.){
                lambda = fabs(Entropy_fix(lambda1,lambdaL1,lambdaR1,0));
                dH[i] += dt/dx[i] * (lambda*alpha1-source);
                dQ[i] += dt/dx[i] * (lambda*alpha1-source)*lambda1;
            }
            if (lambda2<0.){
                lambda = fabs(Entropy_fix(lambda2,lambdaL2,lambdaR2,0));
                dH[i] += dt/dx[i] * (lambda*alpha2+source);
                dQ[i] += dt/dx[i] * (lambda*alpha2+source)*lambda2;
            }
...
double GlobalTimeStep (double *Q, double *H, double *dx) {                  // Calculated as the minimum of the time step for each cell boundary

    //printf("Calculo del paso temporal para la siguiente iteración:\n");
    int i;
    double c, u, dt, dtLocal;
    dt=INFTY;
    //printf("i=0,\tdt=%.3lf\n",dt);
...
//	Write the numbering in the file name with the corresponding extension
    sprintf(filename,"OutputFiles/dataSW_Upwind%d.dat",*PrintCounter);
